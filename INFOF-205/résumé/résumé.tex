...
\DeclareMathOperator{\rg}{rang}  % rang d'une matrice (ou d'une application linéaire)
...
\section{Introduction}
	...
	conditionnement~:
	\[\kappa(d) = \norm {F'(d)}\frac {\norm d}{\norm x} = 1\frac {\norm d}{\norm {d-a}}.\]
	On voit alors que le conditionnement est petit pour $\norm {d-a} \gg 0$. Cependant, pour $d$ proche de $a$, le conditionnement devient très grand.
...
\section{Résolution d'équations non-linéaires}
	...
	\subsection{Méthodes linéarisées}
		Par le développement de Taylor d'ordre 1 (et la formule du reste de Lagrange), autour de $\alpha$, on peut exprimer~:
		\[0 = f(\alpha) = f(x) + f'(\xi)(\alpha-x),\]
		avec $\xi \in [\alpha, x]$. On en déduit~:
		\[\alpha = x - \frac {f(x)}{f'(\xi)}.\]

		Cependant, on ne connait pas $\xi$. Si $q_k$ est une approximation de $f'(\xi)$, alors ceci conduit à la méthode itérative suivante~:
		\[0 = f(x^{(k)}) + (x^{(k+1)} - x^{(k)})q_k\qquad\iff\qquad x^{(k+1)} = x^{(k)} - f(x^{(k)}) \cdot q_k^{-1}.\]

		L'interprétation à y donner est que $x^{(k+1)}$ est l'intersection entre l'axe $OX$ et la droite de pente $q_k = f'(\xi)$ et passant par
		$(x^{(k)}, f(x^{(k)})$. Il est possible de déterminer plusieurs méthodes en changeant $q_k$, l'approximation de la pente de la droite~:

		\begin{enumerate}
			\item la méthode de la corde converge localement d'ordre 1 en ayant~:
				\[q_k \coloneqq \frac {f(b) - f(a)}{b-a}~;\]
			\item la méthode de la sécante pose~:
				\[q_k \coloneqq \frac {f(x^{(k)}) - f(x^{(k-1)})}{x^{(k+1)} - x^{(k)}},\]
				et converge localement d'ordre $p=1.63$ si $f$ est de classe $C^2$ et si $x^{(0)}$ est suffisamment proche de $\alpha$~;
			\item la méthode de la fausse position (ou \emph{regula-falsi}) pose~:
				\[q_k \coloneqq \frac {f(x^{(k)}) - f(x^{\beta(k)})}{x^{(k)} - x^{\beta(k)}},\]
				où $\beta(k)$ représente le plus grand $\ell$ strictement inférieur à $k$ tel que $f(x^{(k)})f(x^{(\ell)}) < 0$ et est globalement
				convergente d'ordre 1~;
			\item la méthode de Newton pose~:
				\[q_k = f'(x^{(k)}),\]
				et est convergente localement d'ordre 2.
		\end{enumerate}

	\subsection{Itérations de point fixe}
		\begin{déf} Soient $X$ un ensemble et $f : X \to X$. Un \emph{point fixe} de $f$ est une valeur $x \in X$ telle que~:
		\[f(x) = x.\]
		\end{déf}

		Chercher une racine de la fonction $f$ est équivalent au problème de recherche d'un point fixe de la fonction $\phi(x) = f(x) + x$.

		\begin{rmq} Il est toujours possible de trouver une telle fonction $\phi$ mais elle n'est pas toujours unique. Toutes les fonctions $F$ telles que
		$F$ est continue et $F(0) = 0$ permettent de formuler~:
		\[\phi(x)  = (F \circ f)(x) + x.\]
		\end{rmq}

		\begin{déf} Soit $x^{(0)}$, on définit pour tout $k \geq 1$ la méthode itérative dite \emph{de point fixe} par~:
		\[x^{(k)} = \phi(x^{(k-1)}).\]
		\end{déf}

		\begin{déf} On appelle $\phi$ la fonction d'itération associée à la méthode.
		\end{déf}

		\begin{rmq} La fonction est \emph{fortement consistante}, c'est-à-dire consistante à toutes les itérations.
		\end{rmq}

		\begin{thm} Soit $\phi$ continue et $(x^{(k)})$ obtenue par une méthode d'itération de point fixe. Si $x^{(k)} \to \alpha$, alors $\alpha$ est un
		point fixe de $\phi$.
		\end{thm}

		\begin{thm} Soit $\phi : [a, b] \to \R$ de classe $C^1$ sur $[a, b]$. Alors~:
		\begin{itemize}
			\item si $\phi([a, b]) = [a, b]$, alors $\phi$ admet un point fixe~;
			\item si de plus il existe $> \lneqq 1$ tel que~:
				\[\forall x \in [a, b] : \abs{\phi(x)} < 1,\]
				alors~:
				\begin{enumerate}
					\item $\phi$ admet un unique point fixe $\alpha \in [a, b]$ et la suite $(x^{(k)})$ converge en $\alpha$ pour tout choix de $x^{(0)}$
						initial~;
					\item \[\lim_{k \to +\infty}\frac {x^{(k+1)} - \alpha}{x^{(k)} - \alpha} = \lim_{k \to +\infty}\phi(\eta^{(k)}) = \phi'(\alpha),\]
						où $\eta^{(k)}$ est entre $\alpha$ et $x^{(k)}$ telle que~:
						\[x^{(k+1)} - \alpha = \phi(x^{(k)}) - \phi(\alpha) = \phi'(\eta^{(k)})(x^{(k)} - \alpha)~;\footnote{Par le théorème de la moyenne.}\]
				\end{enumerate}
			\item si $\forall x \in [a, b] : \abs {\phi (x)} > 1$, alors pour tout $x^{(0)} \neq \alpha$, la suite d'itérations de point fixe $(x^{(k)})$ ne
			converge pas.
		\end{itemize}
		\end{thm}

		\begin{rmq} Ce théorème généralise le théorème du point fixe de Banach.
		\end{rmq}

		\begin{déf} Si pour tout choix de $x^{(0)}$, la suite d'itérations de point fixe converge en $\alpha$, on dit que $\alpha$ est un point attractif de
		$\phi$ et si la suite ne converge pour aucun $x^{(0)}$ différent de $\alpha$ on dit que $\alpha$ est une point répulsif de $\phi$.
		\end{déf}

		Ce théorème nous apprend que s'il existe un tel $K < 1$ et si $\phi'(\alpha) \neq 0$, alors la méthode est globalement convergente d'ordre 1.

		\begin{thm} Si $f$ est de classe $C^{p+1}$ sur un voisinage $I$ de $\alpha$ avec $p \geq 1$ et que~:
		\[\od[i] \phi x(\alpha) = \begin{cases}0 &\text{ si } 1 \leq i \leq p \\K_i(x) \neq 0 &\text{ si }i=p+1\end{cases},\]
		alors la méthode de point fixe associée à $\phi$ est d'ordre $p+1$ et~:
		\[\lim_{k \to +\infty}\frac {x^{(k+1)} - \alpha}{(x^{(k)} - \alpha)^{p+1}} = \frac {\od[p+1] \phi x(\alpha)}{(p+1)!}.\]
		\end{thm}

		\begin{thm}[Théorème de convergence locale d'Ostrowski] Soient $\phi$ différentiable sur un voisinage $I$ de $\alpha$, un point fixe de $\phi$.
		Si $\abs {\phi'(\alpha)} < 1$, alors il existe un rayon de convergence $\delta > 0$ tel que~:
		\[\forall x^{(0)} \in (\alpha \pm \delta) : x^{(k)} \to \alpha.\]
		\end{thm}

		\begin{rmq} Si la dérivée $\abs {\phi'(\alpha)}$ est strictement supérieure à 1, alors la convergence est impossible (et $\alpha$ est un point
		répulsif) et si la dérivée vaut 1, le cas est indéterminé : il est possible d'obtenir convergence ou divergence.
		\end{rmq}

	\subsection{Analyse des méthodes linéarisées}
		\subsubsection{Méthode de la corde}
		La méthode de la corde peut s'écrire de manière itérative de point fixe comme~:
		\[x^{(k+1)} = \phi(x^{(k)},\]
		où $\phi$ est définie par~:
		\[\phi(x) = x - \frac {b-a}{f(b)-f(a)}f(x).\]
		Le critère de convergence locale $\abs {\phi'(\alpha)} < 1$ est équivalent à~:
		\begin{align*}
			-1 &< \phi('\alpha) &&< 1 \\
			-1 &< 1 - \frac {b-a}{f(b)-f(a)}f'(\alpha) &&< 1 \\
			2  &> \frac {b - a}{f(b) - f(a)}f'(\alpha) &&> 0.
		\end{align*}

		Dans ce cas (donc pas si $\phi'(\alpha) = 0$), la converge est linéaire.

		\subsubsection{Méthode de Newton}
		La méthode de Newton peut également s'écrire de manière itérative du point fixe avec~:
		\[\phi(x) = x - \frac {f(x)}{f'(x)}.\]
		On sait $f(\alpha) = 0$, et on suppose $f'(\alpha) \neq 0$. Alors~:
		\[\phi'(\alpha) = 1 - \frac {f'(\alpha)f'(\alpha) - f''(\alpha)f(\alpha)}{(f'(\alpha))^2} = 1 - \frac {f'(\alpha)^2}{f'(\alpha)^2} = 0.\]

		Cette méthode est convergente localement d'ordre $p=2$.

	\subsection{Erreur, incrément et résidu}
		\subsubsection{Erreur et résidu}
		\begin{déf} L'erreur et le résidu sont définis respectivement par~:
		\begin{align*}
			e^{(k)} &= x^{(k)} - \alpha, \\
			r^{(k)} &= f(x^{(k)}) - f(\alpha) = f(x^{(k)}).
		\end{align*}
		\end{déf}

		Si $m$ est la multiplicité de la racine $\alpha$, alors par Taylor, on peut écrire~:
		\[f(x^{(k)}) \simeq f(\alpha) + \frac {f^{(m)}(\alpha)}{m!}(\alpha-x^{(k)})^m,\]
		ou encore, en sachant $f(\alpha) = 0$~:
		\[r^{(k)} \simeq \frac {f^{(m)}(\alpha)}{m!}\left(e^{(k)}\right)^m.\]
		Dès lors, on estimer l'erreur sur base du résidu par~:
		\[\abs {e^{(k)}} \simeq \sqrt[m] {\frac {\abs {r^{(k)}m!}}{\abs {f^{(m)}(\alpha)}}}
		= \sqrt[m] {\frac {m!}{\abs {f^{(m)}(\alpha)}}}\sqrt[m] {\abs {r^{(k)}}}.\]

		Si le critère d'arrêt est basé sur la valeur du résidu (et donc si la méthode s'arrête lorsque $\abs {r^{(k)}} = \abs {f(x^{(k)})} < \varepsilon$ pour
		$\varepsilon > 0$ une tolérance fixée), alors on remarque que pour~:
		\begin{itemize}
			\item $\abs {f'(\alpha)} \simeq 1$, alors $\abs {e^{(k)}} \simeq \abs {r^{(k)}} < \varepsilon$ et donc $\abs{e^{(k)}} \simeq \varepsilon$~;
			\item $\abs {f'(\alpha)} \gg 1$, alors $\abs {e^{(k)}}$ peut être assez grand par rapport à $\varepsilon$ (et donc la solution pas si proche de
				$\alpha$ qu'espéré)~;
			\item $\abs {f'(\alpha)} \ll 1$, alors $\abs {e^{(k)}}$ est très petit par rapport à $\varepsilon$, et donc le test est trop restrictif (peut
				mettre beaucoup de temps à converger).
		\end{itemize}

		\subsubsection{Erreur et incrément}
		On considère la suite $(x^{(k)})$ produite par une méthode de point fixe. On sait que l'on peut écrire~:
		\[e^{(k+1)} = \alpha - x^{(k+1)} = \phi(\alpha) - \phi(x^{(k)}) = \phi'(\eta^{(k)})(\alpha - x^{(k)}) = \phi'(\eta^{(k)})e^{(k)}.\]
		Dès lors~:
		\[x^{(k+1)} - x^{(k)} = (x^{(k+1)} - \alpha) - (x^{(k)} - \alpha) = -e^{(k+1)} + e^{(k)} = (1 - \phi'(\eta^{(k)}))e^{(k)}.\]

		En supposant que $\phi'(\eta^{(k)}) \simeq \phi'(\alpha)$\footnote{On sait que $\eta^{(k)}$ est compris entre $\alpha$ et $x^{(k)}$, or
		$x^{(k)} \to \alpha$, ce qui implique $\eta^{(k)} \to \alpha$.}, on peut approximer (si $\phi'(\alpha) \neq 1$)~:
		\[e^{(k)} = \frac 1{1-\phi'(\alpha)}(x^{(k+1)} - x^{(k)}).\]

		En supposant $f$ continûment dérivable (de classe $C^1$), on peut alors définir un test qui fait arrêter la méthode lorsque
		$\abs {x^{(k+1)} - x^{(k)}} < \varepsilon$ pour $\varepsilon > 0$, une tolérance fixée. À nouveau, cette méthode n'est pas parfaite~:
		\begin{itemize}
			\item si $\phi'(\alpha)$ est proche de 1, le test n'est pas satisfaisant~;
			\item si $\phi'(\alpha) = 0$, alors l'égalité approximative devient parfaite, et donc le test est parfait.
		\end{itemize}
