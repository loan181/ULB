...
% 3rd class
		...
		\subsection{Exemple d'une fonction continue sur $\R$ nulle part dérivable}
			...
			\[\abs {f(\beta_n) - f(\alpha_n)} \geq \left(\frac 34\right)^n\abs {\varphi(4^n\beta_n) - \varphi(4^k\alpha_n)} - \sum_{k=0}^{n-1}\left(\frac 34\right)^k4^{k-n} \geq \frac 12\left(\frac 34\right)^n.\]
	...
	\section{Séries de puissances}
		...
		\subsection{Étude sur le cercle de convergence}
			...
			\begin{thm}[Théorème de Cauchy, version CDI1]\label{thm:CauchyCDI1} Si $\sum_{n \geq 0}a_n$ et $\sum_{n \geq 0}b_n$ convergent absolument,
			...
			\begin{thm}[Théorème de Cauchy, version CDI2] Si $\sum_{n \geq 0}a_n$, $\sum_{n \geq 0}b_n$, et $\sum_{n \geq 0}c_n$ convergent, alors~:
			\[\left(\sum_{n \geq 0}a_n\right)\left(\sum_{n \geq 0}b_n\right) = \sum_{n \geq 0}c_n.\]
			\end{thm}
...
\chapter{Intégration}
	\section{Intégrales absolument convergentes}
		...
		\subsection{Fonctions absolument intégrables vues comme fonction des bornes}
			...
			$M^+, M^- > 0$ tels que~:
			\[\sum_{[u, v] \subset (\minfty, a] \cap I}\int_u^v f \leq M^-\qquad\qquad\text{ et }\qquad\qquad\sup_{[u, v] \subset I \cap [a, \pinfty)}\int_u^v\abs f \leq M^+.\]
			...
			\begin{cor} Si $f$ est abs-int sur $I$ et de classe $C^k$ sur un voisinage de $x_0 \in I$, alors $F$ est de classe $C^{k+1}$ sur un voisinage de $x_0$ et
			on a, sur ce voisinage~:
		...
		\subsection{Critères d'intégration absolue}
			\begin{prp}[Critère de comparaison] Soit $I \subset \R$ un intervalle non-vide et $f, g : I \to \R$ avec~:
			\begin{itemize}
			...
			\begin{prp} Soit $I = [a, b)$, et soit $f : I \to \R$, R-int sur tout segment de $I$. Alors~:
			\begin{enumerate}
