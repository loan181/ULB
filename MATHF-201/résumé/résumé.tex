...
\chapter{Conditions suffisantes d'existence et d'unicité de solutions}
	...
	\section{Flot d'une équation différentielle et intégrale première}
		...
		\subsection{Intégrales premières}
			...
			On en déduit que $t \mapsto H(t, (t))$ est constante sur $I$, son intervalle de définition.
	...
	\section{Condition suffisante d'existence locale}
		\begin{thm}[Théorème de Cauchy-Peano-Arzela] Soient $J \subset \R$ un intervalle et $\Omega \R^d$ ouverts. Soient $f \in C^0(J \times \Omega, \R^d)$ et
...
\chapter{Équations différentielles linéaires}
	...
	\section{Systèmes différentiels linéaires homogènes --- matrice résolvante}
		...
		\end{equation}  % TODO: trouver comment mettre {h \to 0} en-dessous de '='
		\end{prp}
...
\chapter{Séries de Fourier}
	...
	\section{Séries trigonométriques}
		...
		\subsection{Cas de la convergence normale dans $C^0_{2\pi}(\R, \C)$}
			...
			\begin{subnumcases}{}
				...
				\forall n \in \Z : c_n = \frac 1{2\pi}f(t)e^{-int}\dif t\label{eq:Fourier c_n}
			\end{subnumcases}
			\end{prp}
	...
	\section{Série de Fourier d'une fonction périodique}
		...
		\subsection{Coefficients de Fourier d'une fonction périodique}
			...
			\begin{subnumcases}{}
				a_n = \frac 1\pi\int_0^{2\pi}f(t)\cos(nt)\dif t, \\
				b_n = \frac 1\pi\int_0^{2\pi}f(t)\sin(nt)\dif t.
			\end{subnumcases}
			\end{déf}
		...
		\subsection{Les théorèmes de Dirichlet}
			...
			\begin{subnumcases}
				{h : [\pm\pi] \to \C : t \mapsto}
					\frac {f(x+2t)-f(x^+)}2 & si $0 < t < \frac \pi2$ \\
					0 & sinon
			\end{subnumcases}
...
\chapter{Fonctions d'une variable complexe}
	...
	\subsection{Les fonction développables en séries de puissances sont holomorphes}
		\subsubsection{Résultat principal}
			...
			Pour $n=0$, on a $c_0 \cdot 1 - c_0 \cdot 1 = 0$. Pour $n \geq 0$, on a~:
			\begin{equation}
		...
		\subsubsection{Une condition suffisante de développement en série de puissances}
			\begin{thm} Soit $[a, b]$, un segment de $\R$, et soient $\varphi, m \in C^{0,m}([a, b], \C)$. Soit $\Omega \subset \C$, un ouvert tel que
			...
			$\abs {z_0-s} > R$ par définition de $R$.
			...
			La somme étant $C^{0,m}$ est la convergence étant uniforme, il vient~:
			\begin{align}
	...
	\subsection{Les fonctions holomorphes sont développables en séries de puissances}
		\subsubsection{Intégration sur des chemins}
			...
			définie aux points de jonctions), on peut poser $\gamma'(s_k) = z_k$ pour tous les $k$, par exemple $z_k = 0$. $\gamma'$ est alors de classe
			...
			le nombre complexe~:
			\begin{equation}
				\int_\gamma f(\omega)\dif \omega = \int_a^b(f \circ \gamma)(s) \gamma'(s)\dif s,
			\end{equation}
			...
			\begin{align}
				...
				&\leq \norm {f \circ \gamma}_{\infty, \gamma^*} \int_a^b\abs {\gamma'(s)}\dif s = L(\gamma)\norm {f \circ \gamma}_{\infty, \gamma^*}.
			\end{align}
			\end{proof}
			...
			$\varphi \in C^0([a, b], [\tilde a, \tilde b]) \cap C^{1,m}([a, b], [\tilde a, \tilde b])$ strictement croissante sur $[a, b]$.
			\end{déf}

			\begin{rmq} On impose que $\varphi$ soit strictement croissante car une bijection est obligatoirement monotone, ce qui ne laisse que deux
		...
		\subsubsection{Rappels de connexité dans $\C$}
			...
			$E \cap U \neq \emptyset \neq E \cap V$ et $E \cap U \cap V$.

			Elle est dite \textit{connexe} dans le cas contraire.
			\end{déf}

			\begin{déf} $E \subset \C$ est convexe lorsque $\forall a, b \in E : [0, 1] \to \C : t \mapsto a + t(b-a)$ est tracé dans $E$.
			\end{déf}

			\begin{prp} Soit $E \subset \C$ une partie convexe. Elle est alors connexe.
			\end{prp}
